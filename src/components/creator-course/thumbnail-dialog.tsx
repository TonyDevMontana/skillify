...
import { updateCourseThumbnail } from "@/server/actions/update-course-thumbnail";

export function ThumbnailDialog({ courseId }: { courseId: string }) {
  ...
  const { startUpload } = useUploadThing("imageUploader", {
    onClientUploadComplete: async (response) => {
      ...
      const result = await updateCourseThumbnail({ courseId, pictureUrl });
      if (!result.success) {
